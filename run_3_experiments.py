...
def write_infer_script(exp):
    ...
    script = textwrap.dedent(f'''\
        ...
        if not model_files:
            # Also check logs directory root
            model_files = sorted(glob.glob(os.path.join("logs", f"{{model_name}}", "*.pth")))
        if not model_files:
...
def run_subprocess(script_path, description, timeout=7200, env_extra=None):
    ...
    success = result.returncode == 0 or result.returncode % 256 == (2333333 % 256)
    # On macOS, os._exit(2333333) gets truncated to lower 8 bits: 2333333 % 256 = 181
    # But also check raw value
    if result.returncode == 2333333 or result.returncode == -15 or result.returncode == 181:
        success = True
...
def run_experiment(exp_idx, exp):
    ...
    print(f"\n{'='*60}")
    print(f"  Training {model_name} ({TOTAL_EPOCHS} epochs)")
    ...
    # Check success (Applio uses os._exit(2333333))
...
def run_benchmark():
    ...
    script = textwrap.dedent(f'''\
        ...
        print()
        print(f"{{'':<20}} {{'User Sim':>10}} {{'Source Sim':>10}} {{'F0 Corr':>10}} {{'Identity Δ':>10}}")
        print("-" * 60)
        ...
        for name, r in results.items():
            ...
            print(f"{{name:<20}} {{r['sim_user']:>10.4f}} {{r['sim_source']:>10.4f}} {{r['f0_corr']:>10.4f}} {{identity_delta:>+10.4f}}")
        ...
        print(f"Results saved to {MDT_DIR}/benchmark_results.json")
    ''')
...
if __name__ == "__main__":
    ...
    for i, exp in enumerate(EXPERIMENTS):
        ...
        if not success:
            print(f"WARNING: Experiment {exp['model_name']} had issues, continuing...")
