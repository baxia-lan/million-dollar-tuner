"""
Run remaining experiments that v1 didn't get to.
Starts with titan48k and snowie48k, then singer32k if v1 didn't finish it.
Uses 20 epochs with save_every=5 for CPU-feasible training.
"""
import os
import sys
import subprocess
...
# Order: titan and snowie first (v1 is stuck on singer32k),
# then singer32k last (reuse v1's preprocessing, wait for v1 to die)
EXPERIMENTS = [
    {
...
# CPU training: ~3s/step, ~98 steps/epoch = ~5 min/epoch
# 20 epochs * 5 min = 100 min per experiment, fits in 2hr timeout
TOTAL_EPOCHS = 20
SAVE_EVERY_EPOCH = 5
BATCH_SIZE = 4
...
def write_preprocess_script(exp):
    script = textwrap.dedent(f'''\
        ...
        audio_length = pp.process_audio(
            path=target_wav, idx0=0, sid=0,
            cut_preprocess="Automatic", process_effects=True,
            noise_reduction=False, reduction_strength=0.7,
            chunk_len=3.0, overlap_len=0.3, normalization_mode="pre",
        )
...
def write_infer_script(exp):
    script = textwrap.dedent(f'''\
        ...
        audio_opt = vc.pipeline(
            model=hubert_model, net_g=net_g, sid=sid, audio=audio,
            pitch=0, f0_method="rmvpe", file_index="", index_rate=0.0,
            pitch_guidance=bool(use_f0), volume_envelope=1.0,
            version=version, protect=0.5, f0_autotune=False,
...
def run_subprocess(script_path, description, timeout=7200, env_extra=None):
    ...
    try:
        result = subprocess.run(
            [PYTHON, script_path], cwd=APPLIO_DIR, env=env, timeout=timeout,
        )
        elapsed = time.time() - start
        success = result.returncode == 0 or result.returncode == 2333333 or result.returncode == 181
...
def wait_for_port(port, max_wait=7800):
    """Wait until a TCP port is free (i.e., no other training using it)."""
    import socket
    start = time.time()
    while time.time() - start < max_wait:
        ...
        print(f"  Port {port} in use, waiting 30s...")
        sys.stdout.flush()
        time.sleep(30)
    return False


def run_experiment(exp_idx, exp, total_count):
    model_name = exp["model_name"]
    print(f"\n{'#'*60}")
    print(f"# EXPERIMENT {exp_idx+1}/{total_count}: {model_name}")
    ...
    if os.path.isdir(sliced_dir) and len(glob.glob(os.path.join(sliced_dir, "*.wav"))) > 100:
        n = len(glob.glob(os.path.join(sliced_dir, "*.wav")))
        print(f"  Preprocessing already done ({n} slices), skipping...")
    ...
    if (os.path.isdir(extracted_dir) and
        len(glob.glob(os.path.join(extracted_dir, "*.npy"))) > 100 and
    ...
    if existing_models:
        ...
        port = int(exp["master_port"])
        print(f"  Checking port {port}...")
        sys.stdout.flush()
        wait_for_port(port, max_wait=7800)
        ...
        try:
            result = subprocess.run(
                train_cmd, cwd=APPLIO_DIR, env=env, timeout=7200,
            )
...
if __name__ == "__main__":
    print("=" * 60)
    print("3-EXPERIMENT RVC PRETRAIN COMPARISON")
    ...
    for i, exp in enumerate(EXPERIMENTS):
        timings, success = run_experiment(i, exp, len(EXPERIMENTS))
